...
const ENDING_BADGE: Record<string, { label: string; classes: string }> = {
  ...
  lateral: {
    label: "LATERAL",
    classes:
      "bg-[var(--info-subtle)] text-[var(--info)] ring-1 ring-[var(--info)]/30"
  },
  neutral: {
    label: "NEUTRAL",
    classes:
      "bg-[var(--bg-window-sunken)] text-[var(--text-muted)] ring-1 ring-[var(--border)]"
  }
};
...
export function DebriefPage({ onContinue }: DebriefPageProps) {
  ...
  return (
    <div className="p-6 overflow-auto h-full">
      <div className="flex items-center gap-3 mb-2">
        ...
        <span
          className={`px-3 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider ${endingMeta.classes}`}
        >
          {endingMeta.label}
        </span>
      </div>
          ...
          >
            ...
            <div className="space-y-5">
              ...
              {(Object.keys(CATEGORY_LABELS) as ScoreCategory[]).map((cat) => (
                <div
                  key={cat}
                  className="rounded-xl border border-[var(--border)] bg-[var(--bg-window-raised)] overflow-hidden"
                >
                  ...
                  <div className="divide-y divide-[var(--border)]">
                    ...
                    {(actionsByCategory[cat] || []).length === 0 && (
                      <div className="px-4.5 py-3 text-sm text-[var(--text-muted)] italic">
                        No actions recorded.
                      </div>
                    )}
                  </div>
            ...
            {onContinue && (
              <button
                onClick={onContinue}
                className="w-full mt-6 px-6 py-3 rounded-xl text-sm font-semibold text-white transition-all hover:scale-[1.01] active:scale-[0.99]"
      ...
      <TrophyBadge isOpen={showTrophy} onClose={() => setShowTrophy(false)} />
    </div>
  );
}
