...
export function PasswordPuzzle({ onComplete }: PasswordPuzzleProps) {
  ...
  const oldArtifact = useMemo(
    () => parsePasswordArtifact(decisions["login_password_artifact"]),
    [decisions]
  );
  const oldEntropy = oldArtifact?.entropy ?? 0;
  const oldCrack = useMemo(() => crackTime(oldEntropy), [oldEntropy]);
  const oldWasStrong = oldArtifact !== null && oldEntropy >= 60;

  const [phase, setPhase] = useState<Phase>(() => {
    if (!oldArtifact) return "reset";
        ...
        <div className="p-6 max-w-lg mx-auto">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <div className="space-y-4">
              ...
              <button
                onClick={async () => {
                  // Equality is checked against the salted hash — the raw
                  // login password is never stored.
                  const isMatch =
                    oldArtifact !== null &&
                    (await verifyPasswordArtifact(verifyInput, oldArtifact));
                  if (isMatch) {
      ...
      <div className="overflow-auto h-full">
        <div className="p-6 max-w-lg mx-auto">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <p className="text-sm text-[var(--text-secondary)] mb-6 leading-relaxed">
              Forensic analysis found your password hash on the attacker&apos;s
              exfiltration server. Offline cracking recovered the plaintext.
            </p>
            ...
            <div className="p-4 bg-[var(--bg-window-sunken)] rounded-xl border border-[var(--danger)]/40 mb-6">
              ...
              <div className="font-mono text-xs text-[var(--text-secondary)] break-all mb-3">
                {oldArtifact?.hash}
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-[var(--text-secondary)]">Recovered plaintext:</span>
              ...
              <div className="flex items-center justify-between text-sm mt-1">
                <span className="text-[var(--text-secondary)]">Cracked in:</span>
                <span className="font-mono font-semibold" style={{ color: sl.color }}>
                  {oldCrack}
                </span>
              </div>
            ...
            <div className="p-4 rounded-lg border bg-[var(--danger-subtle)] border-[var(--danger)]/35 mb-6">
              <p className="text-sm text-[var(--danger)] leading-relaxed">
                Attackers don&apos;t need your password — they steal the hash and
                crack it offline at billions of guesses per second. At{" "}
                {oldEntropy} bits, yours fell in {oldCrack}. Only high entropy
                makes a stolen hash worthless.
              </p>
            </div>
